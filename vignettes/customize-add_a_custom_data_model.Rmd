...
Once ReviewR has determined data model and version of the user database, it will search within its own namespace for functions that can be used to render the information contained within. These functions must be deveoloped ahead of time. Table functions have a standard naming convention, which allows this look up to occur. Functions used to render patient tables must begin with `{data-model-name}_table_` and will end with the `table_name`.  Two types of table functions must be developed when supporting a new data model, an "all_patients" table and "subject specific" tables. 

#### All Patients Table

`{data-model-name}_table_all_patients`.

Using MIMIC3 as an example, demographic information is contained within the 'PATIENTS' table. An example of an 'all_patients' table would be:
```{r eval=FALSE}
