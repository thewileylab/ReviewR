...
2. Next, using the same R console from above, fork the ReviewR repository to `/srv/shiny-server` with the `usethis` package: 
    ...
    ```
    *This step will fork the most recent version of the ReviewR package from GitHub which includes additional files and development tools not built directly into the package. This includes an `app.R` file which Shiny Server will need to deploy ReviewR. Shiny Server will host all applications that are placed in `/srv/shiny-server` by default.*
