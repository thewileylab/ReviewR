...
#### Download
After Docker Desktop has been installed for your operating system, download the ReviewR Dockerfile to your machine from the following link:

* https://raw.githubusercontent.com/thewileylab/ReviewR/master/Dockerfile
    + Note: This link may open the Dockerfile in your web browser. Right click anywhere in the browser window and select "Save Page As" in order to download the file to your computer. Remember where you save the file!!!

#### Build ReviewR
Using the command line for your operating system, navigate to the location that you downloaded the Dockerfile. For this example, the Dockerfile was downloaded to the home directory, and is the only Dockerfile in that location. 
...
First, we'll need to download the Dockerfile to our server from GitHub:

```{bash, eval=F}
cd ~/
curl -LJO https://raw.githubusercontent.com/thewileylab/ReviewR/master/Dockerfile
```

Next, navigate to the location that you downloaded the Dockerfile. For this example, the Dockerfile was downloaded to the home directory, and is the only Dockerfile in that location. 
...
### Configure nginx
Next, we need to configure NGINX to proxy traffic from web clients to the Docker Container. To accompliish this, we'll need to add a server block to NGINX with information about our container. NGINX server blocks live in `/etc/nginx/sites-available/`. To initialize a server block for our container, issue the following command:
