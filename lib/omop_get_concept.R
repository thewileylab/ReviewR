...
get_concept <- function(table_map, db_connection, concept_table, concept_id, concept_name, table, joinable_id, table_concept, col_name) {
  ...
  user_table(table_map, db_connection, concept_table) %>% 
    ...
    inner_join(user_table(table_map, db_connection, table) %>% 
                 select(user_field(table_map, table, joinable_id), 
                        user_field(table_map, table, table_concept)
                 ), 
               by=setNames(user_field(table_map, table, table_concept), user_field(table_map, concept_table, concept_id))
    ) %>% 
